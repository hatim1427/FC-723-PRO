...
import string
#used to create and manage the bookings database
import sqlite3
...
class Database:
    def __init__(self):
        # connect to the database file and create it if it doesnt exist
        self.connection = sqlite3.connect("bookings.db")
    ...
    def add_booking(self, reference, passport, first_name, last_name, row, column):
    ...
    def remove_booking(self, reference):
        # deletes the passenger record for the given booking reference
        self.cursor.execute(
            "DELETE FROM bookings WHERE booking_reference = ?", (reference,)
        )
        self.connection.commit()
...
class BookingSystem:
    ...
    def __init__(self):
        self.plane = Plane()
        #creat the database connection when the system starts
        self.db = Database()
    ...
    def get_passenger_details(self):
        # asks the user to enter their passenger details
    ...
    def book_seat(self):
        ...
        if not seat.is_bookable():
            ...
            #collect passenger details before confirming the booking
            details = self.get_passenger_details()
            if details is None:
                return
            passport, first_name, last_name = details
            ...
            self.db.add_booking(reference, passport, first_name, last_name, row, col_index + 1)
    ...
    def free_seat(self):
        ...
        if not seat.is_bookable():           
            ...
            # save the reference before freeing so we can delete it from the database
            reference = seat.status
            seat.free()
            # remove the passenger record from the database
            self.db.remove_booking(reference)
    ...
    def book_multiple_seats(self):
        ...
        for col_index, row, seat in seats_to_book:
            # ask for passenger details for each seat individually
            print(f"\n  Passenger details for seat {seat.get_reference()}:")
            details = self.get_passenger_details()
            if details is None:
                return
            passport, first_name, last_name = details
            # make a separate booking code for each seat
            reference = self.generate_booking_reference()
            seat.book(reference)
            # save the passenger details to the database table
            self.db.add_booking(reference, passport, first_name, last_name, row, col_index + 1)
